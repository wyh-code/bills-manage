...
def _parse_xls(filepath: str) -> str:
    ...
    if xls_book.nsheets == 0:
        logger.warning(writeMessage(f"Excel 文件为空: {filepath}"))
        return "[Excel 文件无内容]"
    ...
    for sheet_index in range(xls_book.nsheets):
        ...
        for row_idx in range(sheet.nrows):
            ...
            for col_idx in range(sheet.ncols):
                ...
                if cell_type == 3:  # 日期类型
                    try:
                        date_value = xlrd.xldate_as_datetime(cell_value, xls_book.datemode)
    ...
    logger.info(
        writeMessage(f"Excel 解析成功 (.xls) - 文件: {filepath}, 字符数: {len(content)}")
    )
...
def _parse_xlsx(filepath: str) -> str:
    ...
    logger.info(
        writeMessage(f"Excel 解析成功 (.xlsx) - 文件: {filepath}, 字符数: {len(content)}")
    )
