...
def create_invitation(workspace_id: str, openid: str, role: str, base_url: str) -> dict:
    """
    创建工作空间邀请链接
    ...
    with db_transaction() as db:
        ...
        logger.info(
            writeMessage(
                f"创建工作空间邀请成功 - workspace_id: {workspace_id}, role: {role}, creator: {openid}"
            )
        )
...
def join_by_invitation(token: str, openid: str) -> dict:
    ...
    with db_transaction() as db:
        ...
        use_record = InvitationUse(
            invitation_id=invitation.id, user_openid=openid, invitation_type="workspace"
        )
        db.add(use_record)
        ...
        logger.info(
            writeMessage(
                f"用户通过邀请加入空间 - workspace_id: {invitation.workspace_id}, user: {openid}, role: {invitation.role}"
            )
        )
...
def get_invitations(workspace_id: str, openid: str, status: str = None) -> list:
    """获取空间的邀请列表(仅workspace类型)"""
    # 权限校验(需要editor及以上)
    has_permission, _ = check_workspace_permission(
        workspace_id, openid, required_role="editor"
    )
    if not has_permission:
        raise ValueError("无权限访问该空间")
    ...
    with db_session() as db:
        ...
        for inv in invitations:
            ...
            # 获取创建者信息
            creator = (
                db.query(User).filter(User.openid == inv.created_by_openid).first()
            )
            inv_dict["creator"] = {
            ...
            uses = (
                ...
                .filter(InvitationUse.invitation_id == inv.id)
                .all()
            )
...
def revoke_invitation(workspace_id: str, invitation_id: str, openid: str) -> None:
    """撤销邀请"""
    # 权限校验(需要editor及以上)
    has_permission, _ = check_workspace_permission(
        workspace_id, openid, required_role="editor"
    ...
    with db_transaction() as db:
        ...
        uses = (
            db.query(InvitationUse)
            .filter(InvitationUse.invitation_id == invitation_id)
            .all()
        )
        ...
        logger.info(
            writeMessage(
                f"撤销邀请成功 - invitation_id: {invitation_id}, removed_members: {removed_count}"
            )
        )
