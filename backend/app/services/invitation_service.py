...
from app.utils.permission_checker import check_workspace_permission
...
def get_invitations(workspace_id: str, openid: str, status: str = None) -> list:
    ...
    # 权限校验(需要editor及以上)
    has_permission, _ = check_workspace_permission(workspace_id, openid, required_role='editor')
    if not has_permission:
        raise ValueError('无权限访问该空间')
