...
import requests
from app.utils import get_logger, get_trace_id, generate_token
...
def config(datasource: str) -> dict:
    ...
    try:
        ...
        logger.info(f"请求微信扫码配置 - datasource: {datasource}")
...
def status(datasource: str, state: str) -> dict:
    ...
    try:
        ...
        response.raise_for_status()
        data = response.json()
        logger.info(f"查询扫码状态 - response: {response.text}")
        ...
        with db_transaction() as db:
            ...
            if not user:
                ...
                if is_seed and user.status == "inactive":
                    user.status = "active"
                    logger.info(f"种子用户自动激活 - openid: {openid}")
